                        ...
                        <div class="send-box">
                            <div class="title-box">
                                ...
                                <p>{{$t('otc.publishad.msg2')}}
                                    <a>{{$t('otc.publishad.msg3')}}</a>。</p>
                                <p>{{$t('otc.publishad.msg4')}}
                                    <router-link to="/otc/ad">{{$t('otc.publishad.myad')}}</router-link>
                                    。
                                </p>
                            </div>
                            <!--  -->
                                ...
                                <Form ref="form" :model="form" :rules="ruleValidate" :label-width="90">
                                    ...
                                    <FormItem :label="$t('otc.publishad.exchangecoin')" prop="coin">
                                        <Select v-model="form.coin" :disabled='isId'>
                                            <Option v-for="item in coinList" :value="item.id">{{item.unit}}</Option>
                                        </Select>
                                    </FormItem>
                                    ...
                                    <router-link to="/uc/account" style="padding-left: 90px;">{{$t('otc.publishad.tip2')}}</router-link>
                                    <FormItem :label="$t('otc.publishad.paymode')" prop="payMode">
                                        <Select v-model="form.payMode" multiple style="width:260px">
                                    ...
                                    <FormItem :label="$t('otc.publishad.autoreply')" prop="autodesc" v-show="form.autoReply">
                                        <Input v-model="form.autodesc" type="textarea" :autosize="{minRows: 4,maxRows: 6}" :placeholder="$t('otc.publishad.autoreplytip')"></Input>
                                    </FormItem>
                                    ...
                                    <FormItem>
                                        <Button type="primary" long @click="handleSubmit('form')" :disabled="disAllowBtn">{{$t('otc.publishad.submit')}}</Button>
    ...
    data() {
        ...
        const maxCheck = (rule, value, callback) => {
            ...
            if (!value || value == 0) {
                ...
                callback(new Error(this.$t('otc.publishad.warning6') + priceNow + 'CNY！'));
            } else {
                callback();
            }
        };
        ...
        return {
            ...
            ruleValidate: {
                ...
                coin: [
                    { required: true, message: this.$t('otc.publishad.inputtip1'), trigger: 'change' }
                ],
                country: [
                    { required: true, message: this.$t('otc.publishad.inputtip2'), trigger: 'change' }
                ],
                rmb: [
                    { required: true, message: this.$t('otc.publishad.inputtip2'), trigger: 'change' }
                ],
                premisePrice: [
                    { validator: premisePriceCheck, message: this.$t('otc.publishad.inputtip3'), trigger: 'blur' }
                ],
                fixedPrice: [
                    { validator: fixedPCheck, message: this.$t('otc.publishad.inputtip4'), trigger: 'blur' }
                ],
                number: [
                    { required: true, validator: numberCheck, message: this.$t('otc.publishad.inputtip5'), trigger: 'blur' }
                ],
                timeLimit: [
                    { required: true, validator: timeLimitCheck, message: this.$t('otc.publishad.inputtip6'), trigger: 'blur' },
                ],
                payMode: [
                    { required: true, type: 'array', min: 1, message: this.$t('otc.publishad.inputtip7'), trigger: 'change' },
                ],
                minLimit: [
                ...
                priceW: [
                    { required: true, message: this.$t('otc.publishad.inputtip8'), trigger: 'blur' }
                ],
            },
            payModeList: [
                {
