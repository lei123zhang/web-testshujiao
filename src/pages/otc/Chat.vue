<template>
    <div class="content-wrap">
        <div class="container chat-in-box" id="List">
            <p style="padding: 10px 0 10px 50px;font-size: 16px;"><router-link to="/otc/order">{{$t('otc.myorder')}}</router-link></p>
            <Row class="chat-in">
                <Col span="4">
                        ...
                        <div class="bottom-btn">
                            <div style="padding-top:20px;">
                                ...
                                <div v-show="statusBtn==1&&tradeType==0">
                                    <Button type="primary" @click="modal1 = true">{{$t('otc.chat.orderstatus_1')}}</Button>
                                    <Button @click="modal2 = true" type="error">{{$t('otc.chat.orderstatus_4')}}</Button>
                                </div>
                                <div v-show="statusBtn==2&&tradeType==0">
                                    <Button type="primary" @click="modal4 = true">{{$t('otc.chat.orderstatus_2')}}</Button>
                                    <Button @click="modal2 = true" type="error">{{$t('otc.chat.orderstatus_4')}}</Button>
                                </div>
                                <div v-show="statusBtn==2&&tradeType==1">
                                    <Button type="primary" @click="modal5 = true">{{$t('otc.chat.orderstatus_3')}}</Button>
                                    <Button @click="modal4 = true" type="error">{{$t('otc.chat.orderstatus_5')}}</Button>
                                </div>
...
export default {
    ...
    methods: {
        ...
        ok5() {
            ...
            this.$http.post(this.host + '/otc/order/release', params).then(response => {
                var resp = response.body;
                if (resp.code == 0) {
                    this.$Message.success(resp.message);
                    this.getDetail()
