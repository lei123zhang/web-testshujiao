            ...
            <div class="bill_box rightarea padding-right-clear record">
                <div class="col-xs-12 rightarea-con">
                    <div class="form-group">
                        ...
                        <Button type="primary" @click="queryOrder" style="padding: 6px 50px;margin-left:10px;">{{$t('asset.record.search')}}</Button>
                    </div>
...
export default {
    ...
    data() {
        return {
            ...
            recordType: [
                {
                    value: 0,
                    label: this.$t('asset.record.charge')
                },
                {
                    value: 1,
                    label: this.$t('asset.record.pickup')
                },
                {
                  value:2,
                  label:this.$t('asset.record.transaccount')
                },
                {
                  value:3,
                  label:this.$t('asset.record.exchange')
                },
                {
                  value:4,
                  label:this.$t('asset.record.otcbuy')
                },
                {
                  value:5,
                  label:this.$t('asset.record.otcsell')
                },
            ],
            pageSize:10,
            page:0,
    ...
    methods: {
      ...
      queryOrder(){
          ...
          params['pageNo'] = 0;
          params['pageSize'] = 10;
          if (this.recordValue != "") params['type'] = this.recordValue;
    ...
    computed: {
        tableColumnsRecord() {
            ...
            var that = this;
            columns.push({
                title: this.$t('asset.record.chargetime'),
                key: 'createTime',
            });
            columns.push({
                title: this.$t('asset.record.type'),
                key: 'type',
                render:function(h,params){
                    if(params.row.type == 0){
                        return that.$t('asset.record.charge');
                    }
                    else if(params.row.type == 1){
                        return that.$t('asset.record.pickup');
                    }
                }
            });
            columns.push({
                title: this.$t('asset.record.num'),
                key: 'amount',
            });
            columns.push({
                title: '手续费',
            ...
            columns.push({
                title: this.$t('asset.record.status'),
                key: 'status',
                render:(h,params)=>{
                    return that.$t('asset.record.finish');
                }
            });
            return columns;
        }
