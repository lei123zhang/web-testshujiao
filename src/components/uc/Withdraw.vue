        ...
        <div class="nav-right col-xs-12 col-md-10 padding-right-clear">
            <div class="bill_box rightarea padding-right-clear record">
                <section class="trade-group merchant-top">
                    ...
                    <span class="tips-word">{{$t('asset.withdraw.pickup')}}</span>
                    <span class="head-group">
                        <router-link to="/finance/withdraw/address">{{$t('asset.withdraw.addressmanager')}}</router-link>
                    </span>
                </section>
                <section>
                    <div class="table-inner action-box">
                        ...
                        <div class="action-inner">
                            ...
                            <div class="inner-box">
                                <div class="form-group form-address">
                                    <label for="controlAddress" class="controlAddress describe">{{$t('asset.withdraw.address')}}</label>
                        ...
                        <div class="form-group-container">
                            <div class="form-group form-amount">
                                <label class="label-amount"> {{$t('asset.withdraw.num')}}
                                    <p class="label-fr">
                                        <span>【{{$t('asset.withdraw.avabalance')}}】：
                                            <span class="label-pointer" id="valueAvailable">{{currentCoin.balance}}</span>
                                        </span>
                                        <span v-if="currentCoin.enableAutoWithdraw == 0">【{{$t('common.tip')}}】：{{$t('asset.withdraw.msg1')}} {{currentCoin.threshold}} {{$t('asset.withdraw.msg2')}}</span>
                                        <span>
                                            <a href="javascript:;" id="levelUp" style="display: none;">{{$t('asset.withdraw.increase')}}</a>
                                        </span>
                                    </p>
                                </label>
                                <div class="input-group">
                                    <Poptip trigger="focus" :content="$t('asset.withdraw.tip1')+currentCoin.minAmount+' ,'+$t('asset.withdraw.tip2')+currentCoin.maxAmount" style="width: 100%;">
                                        <InputNumber @on-change="computerAmount" v-model="withdrawAmount" :placeholder="$t('asset.withdraw.numtip1')" size="large" style="width: 900px;"></InputNumber>
                        ...
                        <div class="form-group-container form-group-container2">
                            <div class="form-group form-fee">
                                <label class="label-amount"> {{$t('asset.withdraw.fee')}}
                                    <p class="label-fr">
                                        <span>{{$t('asset.withdraw.range')}}：{{currentCoin.minTxFee}} - {{currentCoin.maxTxFee}}</span>
                                    </p>
                                </label>
                                <div class="input-group">
                                    ...
                                    <Poptip v-else trigger="focus" :content="$t('asset.withdraw.tip1')+currentCoin.minTxFee+$t('asset.withdraw.tip1')+currentCoin.maxTxFee" style="width: 100%;">
                            ...
                            <div class="form-group">
                                <label>{{$t('asset.withdraw.arriamount')}}</label>
                                <div class="input-group">
                                    <InputNumber v-model="withdrawOutAmount" :placeholder="$t('asset.withdraw.arriamount')" readonly size="large" style="width: 100%;"></InputNumber>
                        ...
                        <div class="action-foot">
                            <Button id="withdrawSubmit" long size="large" type="primary" style="height:40px;" @click="apply">{{$t('asset.withdraw.pickup')}}</Button>
                        </div>
                        <div class="action-content pt10">
                            <div class="action-body">
                                <p class="acb-p1">{{$t('common.tip')}}</p>
                                <p class="acb-p2">• {{$t('asset.withdraw.msg3')}}：{{currentCoin.minAmount}}{{$t('asset.withdraw.msg4')}}。<br>• {{$t('asset.withdraw.msg5')}}<br>• {{$t('asset.withdraw.msg6')}} </p>
                            </div>
                        </div>
                        <div class="action-content">
                            <div class="action-body">
                                <p class="acb-p1">{{$t('asset.withdraw.record')}}</p>
...
export default {
    ...
    methods: {
        ...
        valid() {
            if (this.coinType == '') {
                this.$Message.error(this.$t('asset.withdraw.symboltip'));
                return false
            } else if (this.withdrawAdress == '') {
                this.$Message.error(this.$t('asset.withdraw.addresstip'));
                return false
            } else if (this.withdrawAmount == '' || this.withdrawAmount == 0 || (this.withdrawAmount - 0) < this.minAmount) {
                this.$Message.error(this.$t('asset.withdraw.numtip2') + this.minAmount);
                return false
            } else if ((this.withdrawAmount - 0) < this.withdrawFee) {
                this.$Message.error(this.$t('asset.withdraw.numtip3'));
                return false
                ...
                this.$Message.error(this.$t('asset.withdraw.feetip1') + this.currentCoin.minTxFee + ' , '+this.$t('asset.withdraw.feetip2') + this.currentCoin.maxTxFee);
                return false
            } else {
                return true
            }
    ...
    computed: {
        tableColumnsWithdraw() {
            let columns = [];
            columns.push({
                title: this.$t('asset.withdraw.time'),
                width: 180,
                key: 'createTime',
            });
            columns.push({
            ...
            columns.push({
                title: this.$t('asset.withdraw.num'),
                key: 'totalAmount',
            });
            columns.push({
                title: this.$t('asset.withdraw.fee'),
                key: 'fee',
            });
            columns.push({
                title: this.$t('asset.withdraw.status'),
                key: 'status',
                render: (h, params) => {
                    let text = ''
                    if (params.row.status == 0) {
                        text = this.$t('asset.withdraw.status_1');
                    } else if(params.row.status == 1){
                        text = this.$t('asset.withdraw.status_2');
                    }
                    else if(params.row.status == 2){
                        text = this.$t('asset.withdraw.status_3');
                    }
                    else if(params.row.status == 3){
                        text = this.$t('asset.withdraw.status_4');
                    }
                    return h('div', [
                        h('p', text),
                    ]);
