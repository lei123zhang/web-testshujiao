        ...
        <div class="nav-right col-xs-12 col-md-10 padding-right-clear">
            <div class="bill_box rightarea padding-right-clear record">
                <section class="trade-group merchant-top">
                    ...
                    <span class="tips-word">{{$t('asset.recharge.recharge')}}</span>
                </section>
                <section>
                    <div class="table-inner action-box open">
                        ...
                        <div class="action-inner">
                            <div class="inner-left">
                                <p class="describe">{{$t('asset.recharge.symbol')}}</p>
                            ...
                            <div class="inner-box deposit-address">
                                <p class="describe">{{$t('asset.recharge.address')}}</p>
                                <div class="title">
                                    <Input v-model="qrcode.value" readonly style="width: 400px"></Input>
                                    <a v-clipboard:copy="qrcode.value" v-clipboard:success="onCopy" v-clipboard:error="onError" href="javascript:;" id="copyBtn" class="link-copy">{{$t('asset.recharge.copy')}}</a>
                                    <a id="showQRcode" class="link-qrcode" href="javascript:;" @click="showEwm"> {{$t('asset.recharge.qrcode')}}
                        ...
                        <div class="action-content">
                            <div class="action-body">
                                ...
                                <p class="acb-p2">• {{$t('asset.recharge.msg1')}}<br>• {{$t('asset.recharge.msg2')}}<br>• {{$t('asset.recharge.msg3')}}<br>• {{$t('asset.recharge.msg4')}}<br>• {{$t('asset.recharge.msg5')}}</p>
                            </div>
                        </div>
                        <div class="action-content">
                            <div class="action-body">
                                <p class="acb-p1">{{$t('asset.recharge.record')}}</p>
...
export default {
    ...
    methods: {
        ...
        onCopy(e) {
            this.$Message.success(this.$t('asset.recharge.copysuccess') + e.text);
        },
        onError(e) {
            this.$Message.error(this.$t('asset.recharge.copysuccess'));
        },
        changeCoin(value) {
    ...
    computed: {
        tableColumnsRecharge() {
            let columns = [];
            columns.push({
                title: this.$t('asset.recharge.time'),
                key: 'createTime',
            });
            columns.push({
                title: this.$t('asset.recharge.symbol'),
                key: 'symbol',
            });
            columns.push({
                title: this.$t('asset.recharge.address'),
                key: 'address',
            });
            columns.push({
                title: this.$t('asset.recharge.amount'),
                key: 'amount',
            });
            return columns;
        }
